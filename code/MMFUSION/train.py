...
from .data import batch_pad_collate_fn, input_pipeline_train
...
def load_train_state(path : Path):
    ...
    model = MMFUSION(checkpoint["n_classes"], load_pretrained=False)
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded train state for MMFUSION, last epoch:", checkpoint["epoch"])
    return {
        "model" : model,
        "train_function" : partial(train_model, model=model, input_pipeline = input_pipeline_train,
...
def train_model(model : MMFUSION , train_ds : torch.utils.data.Dataset,
    ...
    Args:
        ...
        train_checkpointing (bool, optional): Whether to save the model each epoch. Defaults to False.
    ...
    model.train()
    print("Training MMFUSION model")
    
    if train_checkpointing != 0:
        assert save_base_folder != None, "Train checkpointing was requested but no folder was provided"
        model_save_folder = save_base_folder / "MMFUSION_train"
        model_save_folder.mkdir(exist_ok=True)
        base_save_name = ( model_save_folder / "MMFUSION").resolve()
    
    n_epochs = 30
    loss_fn = torch.nn.CrossEntropyLoss()
    

    # Freeze part of HUBERT and let ConvNext learn
    ...
    optimizer = torch.optim.AdamW([
        {'params': model.hubert.base_model.encoder.parameters(), 'lr':10e-6},
        {'params': model.pretrained_cnn.parameters(), 'lr':10e-6},

        {'params': model.head.parameters(), 'lr' : 10e-5},    
   
        ], lr=10e-5, weight_decay=1e-5)

    if optimizer_state_dict != None:
        optimizer.load_state_dict(optimizer_state_dict)

    # Decay lr by 10 every 5 epochs with no improvement
    # If no validation ds is provided use a fixed schedule scheduler
    if val_ds is None:
    ...
    for epoch in range(starting_epoch, n_epochs+1):
        ...
        for batch, batch_data in enumerate(batched_ds):
            ...
            X_w, X_mfcc, X_spectro = input_pipeline(batch_data["soundwave"])
            X_w = X_w.to(device)
            X_mfcc = X_mfcc.to(device)
            X_spectro = X_spectro.to(device)
            ...
            logits = model(X_w, X_mfcc, X_spectro)
