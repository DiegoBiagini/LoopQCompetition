...
from .data import input_pipeline_train
import numpy as np
...
def load_train_state(path : Path):
    ...
    model = FC_MFCC(checkpoint["n_classes"])
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded train state for FC_MFCC, last epoch:", checkpoint["epoch"])
    return {
        "model" : model,
        "train_function" : partial(train_model, model=model, input_pipeline = input_pipeline_train,
...
def train_model(model : FC_MFCC , train_ds : torch.utils.data.Dataset,
    ...
    optimizer = torch.optim.AdamW([
            ...
            {"params" : model.middle.parameters(), "weight_decay":10e-6}, # Add weight decay only to the middle part
    ...
    # Decay lr by e^-0.15 every 20 epochs after epoch 50
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=math.pow(math.e,-0.15), verbose=True)
    # Decay lr by 10 every 10 epochs with no improvement
    ...
    for epoch in range(starting_epoch, n_epochs+1):
        ...
        batched_ds = DataLoader(train_ds, batch_size=64, shuffle=True, num_workers = 2, pin_memory=True)
        ...
        val_acc = None
        if val_ds != None:
            batched_val_ds = DataLoader(val_ds, batch_size=64, shuffle=True, num_workers = 2, pin_memory=True)
