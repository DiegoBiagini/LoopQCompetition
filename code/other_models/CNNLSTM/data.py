...
SPECTRO_LENGTH = math.floor((SAMPLE_RATE*CLIP_LENGTH)/HOP_FRAMES) + 1

def input_pipeline_train(batch_waveform : torch.Tensor):
    # Compute the log mel spectrogram
    melsp = torchaudio.transforms.MelSpectrogram(sample_rate=SAMPLE_RATE,
                n_fft =WINDOW_FRAMES, n_mels = MEL_BINS, hop_length = HOP_FRAMES,
                mel_scale = "htk")
    to_db = torchaudio.transforms.AmplitudeToDB(top_db=80)
    return to_db(melsp(batch_waveform))
    

def input_pipeline_inference(batch_waveform : torch.Tensor): # TODO
    melsp = torchaudio.transforms.MelSpectrogram(sample_rate=SAMPLE_RATE,
                n_fft =WINDOW_FRAMES, n_mels = MEL_BINS, hop_length = HOP_FRAMES,
                mel_scale = "htk")
    to_db = torchaudio.transforms.AmplitudeToDB(top_db=80)
...
def load_inference_model(path : Path):
    ...
    model = CNNLSTM2(checkpoint["n_classes"], SPECTRO_LENGTH, MEL_BINS, inference=True)
