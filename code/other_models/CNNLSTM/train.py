...
from .data import input_pipeline_train, SPECTRO_LENGTH, MEL_BINS
...
def init_model(n_classes : int):
    ...
    """
    model = CNNLSTM(n_classes, SPECTRO_LENGTH, MEL_BINS)
    return {
        "model" : model,
...
def load_train_state(path : Path):
    ...
    model = CNNLSTM(checkpoint["n_classes"], checkpoint["init_w"], checkpoint["init_h"])
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded train state for CNNLSTM2, last epoch:", checkpoint["epoch"])
    return {
        "model" : model,
        "train_function" : partial(train_model, model=model, input_pipeline = input_pipeline_train,
...
def train_model(model : CNNLSTM , train_ds : torch.utils.data.Dataset,
    ...
    # Decay lr by 0.5 every 5 epochs with no improvement
